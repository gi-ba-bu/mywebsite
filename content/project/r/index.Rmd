...
# Using <mark>ggplot()</mark> and <mark>dplyr</mark > in <mark>standard linear regression</mark>.

In this section I take a look at data related to the Covid-19 epidemic. I will try to answer the following questions: "which EU country was the hardest hit by Covid-19?" and "is the wealth of countries a good predictor of deaths due to Covid-19?"

The data that I use is imported from the European Centre for Disease Prevention and Control (ECDC) website and does not include data about the United Kingdom. First, I will show a table for the cumulative deaths and cases for each EU country for which data is available. Numbers refer to the pandemic from its beginning to mid august 2021. Second, for each country, I import information about its GDP from the Eurostat website.

The R code used in this study is displayed in code chunks below; the code output is rendered after each output command. In the next section, you will find that:
