...
# Using <mark>ggplot()</mark> and <mark>dplyr</mark > in standard linear regression.

In this section I take a look at data related to the Covid-19 epidemic. The data is imported from the European Centre for Disease Prevention and Control (ECDC) website and does not include data about the United Kingdom. First, I show a table for the cumulative deaths and cases for each EU country for which data is available. Numbers refer to the pandemic from its beginning to mid_august 2021.

Here I will try to answer the following question, ask yourself "which EU country was the hardest hit by Covid-19?" and "is the wealth of countries a good predictor of deaths due to Covid-19?"
...
Finally, a trend seems to emerge in which countries with a low per-capita GDP tend to accrue more deaths per million people.
```{r}
library(tidyverse)
library(kableExtra)
library(scales)
...
---
# Press freedom and Covid-19 deaths: a correlation
...
In the code below, the following steps are completed:

- The data is imported from separate sources and merged

- A scatter plot with line of best fit is displayed

- A summary statistics of the linear regression is printed

- A q-q plot for the residuals is printed 
...
The value for R squared is 0.5552 (0.5359 when adjusted). As rule of thumb, this implies that just over 50% of the variance of the dependent variable (deaths) is explained by the dependent variable (GDP) alone.  
